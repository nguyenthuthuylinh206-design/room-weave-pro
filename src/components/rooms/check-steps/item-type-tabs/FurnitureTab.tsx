...
export function FurnitureTab({
  ...
  const allowedActions = (() => {
    if (checkType === 'checkout' && phase && config.phase1Actions && config.phase2Actions) {
      return phase === 1 ? config.phase1Actions.furniture : config.phase2Actions.furniture
    }
    return config.furnitureActions
  })()
                        ...
                        <div className="space-y-2 p-2 bg-amber-50 rounded-lg border border-amber-200 mt-1">
                          ...
                          >
                            <div className="flex items-center space-x-1.5">
                              <RadioGroupItem value="repairable" id={`fr-${item.item_id}`} />
                              <Label htmlFor={`fr-${item.item_id}`} className="text-xs">Sửa (50%)</Label>
                            </div>
                            <div className="flex items-center space-x-1.5">
                              <RadioGroupItem value="replacement_needed" id={`frn-${item.item_id}`} />
                              <Label htmlFor={`frn-${item.item_id}`} className="text-xs">Thay (100%)</Label>
                            </div>
                          </RadioGroup>
