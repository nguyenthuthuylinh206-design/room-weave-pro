...
export function FurnitureTab({
  ...
  const handleStartAction = (itemId: string, type: 'lost' | 'damaged') => {
    ...
    // Set default damage cost based on item price
    const itemPrice = itemPrices[itemId] || 0
    setDamageCost(type === 'damaged' ? Math.round(itemPrice * 0.5) : itemPrice) // 50% for repair, 100% for lost
  }
  ...
  const handleDamageTypeChange = (value: 'repairable' | 'replacement_needed') => {
    ...
    // Auto-calculate cost: 50% for repair, 100% for replacement
                        ...
                        <div className="space-y-1.5">
                          ...
                          >
                            <div className="flex items-center space-x-1.5">
                              ...
                              <Label htmlFor="repairable" className="text-xs cursor-pointer">Cần sửa (50%)</Label>
                            </div>
                            <div className="flex items-center space-x-1.5">
                              <RadioGroupItem value="replacement_needed" id="replacement_needed" />
                              <Label htmlFor="replacement_needed" className="text-xs cursor-pointer">Cần thay (100%)</Label>
                            </div>
                          </RadioGroup>
                        </div>
