...
| F-FSM-02 | 🟡 | `room_bookings.status` & `housekeeping_tasks.status`. **2026-05-10 (đợt sau)**: audit 4 file `useBookingActions.ts`, `useCheckoutInspection.ts`, `useTaskQc.ts`, `BookingsPage.tsx` → chỉ 1 vi phạm thực ở `useCheckoutInspection.cancelInspection` (cancel housekeeping_tasks); đã refactor sang `transition_task_status('cancelled')`. Các `.update()` còn lại trên `room_bookings` chỉ chạm `notes`/`amount_paid`/`payment_status`, không phải `status`. **Còn 2 vi phạm ngoài scope**: `useHousekeepingTasks.ts:481`, `RoomCheckPage.tsx:918`. ESLint chặn mọi vi phạm mới. | ESLint sweep | Refactor 2 file còn lại + revoke RLS column UPDATE (status) |
