-- Fix handover_batch RPC: insert one inventory_transactions row per item with all required NOT NULL fields.
-- Previous version inserted a single header row missing item_id/quantity/quantity_before/quantity_after,
-- and referenced non-existent table inventory_transaction_items. Also used invalid transaction_type 'outbound'.
...
BEGIN
  ...
  -- Stock check (only when no manual adjustments)
  ...
  -- Base transaction code
  v_transaction_code_base := 'TXN-HDO-' || to_char(now(), 'YYYYMMDD-HH24MISS') || '-' || substring(gen_random_uuid()::text, 1, 4);

  -- Aggregate quantity per item across all rooms in this batch, then insert one transaction row per item
  FOR v_item IN
    SELECT doi.item_id,
  ...
  LOOP
    -- Determine actual qty (sum from adjustments if provided)
    IF p_adjustments IS NOT NULL THEN
      ...
      -- Update quantity_actual on each item row for this item_id in this batch
    ...
    -- Insert one inventory_transactions row per item
  ...
  -- Update order status -> released, attach first transaction id
