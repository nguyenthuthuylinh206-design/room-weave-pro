...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE u.id = auth.uid() AND u.tenant_id = v_tenant
  ) THEN
  ...
  next_bk AS (
    ...
    WHERE rb.hotel_id = p_hotel_id AND rb.tenant_id = v_tenant
      ...
      AND (rb.check_in_date + COALESCE(rb.expected_check_in_time, '14:00'::time))
          BETWEEN now() - interval '2 hours' AND now() + interval '48 hours'
  ...
  hk_tasks AS (
    ...
    WHERE hotel_id = p_hotel_id AND tenant_id = v_tenant
  ...
  FROM (
    SELECT rl.floor_key,
      jsonb_agg(
        jsonb_build_object(
          ...
          'status', rl.status,
          'open_tasks', COALESCE(ht.cnt, 0) + COALESCE(mt.cnt, 0),
          'open_hk_tasks', COALESCE(ht.cnt, 0),
          'open_maintenance', COALESCE(mt.cnt, 0),
    ...
    LEFT JOIN hk_tasks ht ON ht.room_id = rl.id
    LEFT JOIN mt_reqs mt ON mt.room_id = rl.id
