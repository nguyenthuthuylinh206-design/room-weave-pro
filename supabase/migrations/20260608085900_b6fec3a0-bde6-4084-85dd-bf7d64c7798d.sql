...
DECLARE
  ...
  _old_out date;
  _old_total numeric;
  v_room_base_price numeric;
  v_room_monthly_price numeric;
  v_new_nights integer;
  v_new_months numeric;
  v_new_total numeric;
BEGIN
  ...
  _old_total := _b.total_amount;

  SELECT base_price, monthly_price
    INTO v_room_base_price, v_room_monthly_price
  FROM public.rooms WHERE id = _b.room_id;

  IF _b.booking_type = 'monthly' THEN
    v_new_months := (_new_check_out_date - _new_check_in_date)::numeric / 30.0;
    v_new_total := v_new_months * COALESCE(v_room_monthly_price, 0);
  ELSE
    v_new_nights := _new_check_out_date - _new_check_in_date;
    v_new_total := v_new_nights * COALESCE(v_room_base_price, 0);
  END IF;
  ...
  PERFORM public.log_state_transition(
    ...
    p_context    => jsonb_build_object(
      ...
      'old_total_amount',   _old_total,
      'new_total_amount',   v_new_total,
      'booking_type',       _b.booking_type
    )
  );
