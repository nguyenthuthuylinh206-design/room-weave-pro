-- ============================================
-- FORCE DROP and RECREATE complete_registration
-- ============================================

-- Drop all versions of the function
DROP FUNCTION IF EXISTS complete_registration(UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER);
DROP FUNCTION IF EXISTS complete_registration;

-- Recreate with correct schema (NO tenant_id in user_roles)
...
BEGIN
  ...
  -- 6. Create user_hotels relationship (NO tenant_id if not in schema)
  INSERT INTO user_hotels (user_id, hotel_id, is_default, is_active)
  VALUES (p_user_id, v_hotel_id, true, true)
...
COMMENT ON FUNCTION complete_registration IS 'Completes user registration by creating tenant, hotel, and default data. SECURITY DEFINER to bypass RLS.';