...
BEGIN
  ...
  SELECT
    ...
    COALESCE(x.base_price, 0)::numeric AS base_price,
    COALESCE(x.final_price, 0)::numeric AS final_price,
    COALESCE(jsonb_array_length(x.seasonals), 0) > 0 AS has_seasonal,
    (x.override_price IS NOT NULL) AS has_override,
    COALESCE(x.is_closed, false) AS is_closed
  FROM public.room_types rt
  LEFT JOIN LATERAL (
    SELECT *
    FROM public.resolve_daily_prices_bulk(rt.id, CURRENT_DATE, CURRENT_DATE, p_apply_to, p_hotel_id)
    LIMIT 1
  ) x ON true
