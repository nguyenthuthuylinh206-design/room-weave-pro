-- Patch calculate_booking_price: 
-- - overwrite + set_rate / fixed_amount => giá tuyệt đối / đơn vị (đêm/giờ/tháng)
-- - overwrite + percent => giá nền * (1 + %/100) * units * weekend_mult
-- - add_on giữ nguyên semantics
...
BEGIN
    ...
    LOOP
      IF v_season_row.mode = 'overwrite' AND NOT v_overwrite_applied THEN
        IF v_season_row.adjust_type = 'percent' THEN
          -- Ghi đè theo % giá nền: subtotal = base * (1 + %/100)
          v_subtotal := v_base * (1 + v_season_row.adjust_value / 100.0) * v_weekend_mult;
        ELSE
          -- set_rate hoặc fixed_amount trong overwrite => giá tuyệt đối / đơn vị
  ...
  RETURN jsonb_build_object(
    ...
    'seasonals', v_season,
    'early_checkin_fee', v_early,
    'late_checkout_fee', v_late,
    'total', v_total
  );
END;
$$;
