-- Function to update distribution order (only when pending)
-- Allows: change assigned_to, notes, add/remove rooms, add/remove items
...
BEGIN
  ...
  IF p_rooms IS NOT NULL AND jsonb_array_length(p_rooms) > 0 THEN
    ...
    LOOP
      ...
      -- Delete items not in new list and restore stock
      FOR v_item_record IN
        SELECT doi.id, doi.item_id, doi.quantity, i.quantity_in_stock
      ...
      LOOP
        ...
        IF NOT EXISTS (
          ...
          UPDATE items
          SET quantity_in_stock = quantity_in_stock + v_item_record.quantity
      ...
      LOOP
        ...
        IF FOUND THEN
          ...
          IF v_quantity != v_old_quantity THEN
            ...
            UPDATE items
            SET quantity_in_stock = quantity_in_stock + v_old_quantity - v_quantity
            WHERE id = v_item_id;
    ...
    -- Delete rooms not in new list (and restore stock)
    ...
    LOOP
      -- Restore stock for all items in this room
      UPDATE items i
      SET quantity_in_stock = i.quantity_in_stock + doi.quantity
      FROM distribution_order_items doi
      WHERE doi.distribution_order_room_id = v_room_order_id
