-- Fix delete_inventory_transaction RPC to also rollback warehouse_stock
...
BEGIN
  ...
  -- Get warehouse_id from transaction (from_warehouse_id for outbound, to_warehouse_id for inbound)
  ...
  IF v_transaction.transaction_type IN ('in', 'inbound') THEN
    ...
    -- Also update warehouse_stock if warehouse was specified
    IF v_warehouse_id IS NOT NULL THEN
      UPDATE warehouse_stock
      SET 
        quantity = GREATEST(0, COALESCE(quantity, 0) - v_transaction.quantity),
        last_updated = NOW()
  ...
  ELSIF v_transaction.transaction_type IN ('out', 'outbound') THEN
    ...
    IF v_warehouse_id IS NOT NULL THEN
      UPDATE warehouse_stock
      SET 
        quantity = COALESCE(quantity, 0) + v_transaction.quantity,
        last_updated = NOW()
      WHERE warehouse_id = v_warehouse_id AND item_id = v_transaction.item_id;
    END IF;
  END IF;
