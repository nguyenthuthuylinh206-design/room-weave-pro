-- Fix complete_room_delivery function to remove updated_at reference
...
BEGIN
  ...
  LOOP
    ...
    UPDATE items
    SET 
      quantity_pending = COALESCE(quantity_pending, 0) - v_confirmed_qty,
    ...
    IF v_missing_qty > 0 THEN
      UPDATE items
      SET 
        quantity_pending = COALESCE(quantity_pending, 0) - v_missing_qty,
