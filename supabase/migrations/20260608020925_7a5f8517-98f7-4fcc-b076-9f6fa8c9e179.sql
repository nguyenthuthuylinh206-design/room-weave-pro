...
-- Bỏ overload cũ (13 args) để tránh ambiguity
DROP FUNCTION IF EXISTS public.perform_checkout(uuid, uuid, numeric, numeric, numeric, numeric, numeric, numeric, numeric, text, jsonb, numeric, date);

-- Tạo lại với 10 args. Server tự tính vat_amount/service_fee_amount/total_amount
-- từ p_subtotal và vat_rate/service_fee_rate đọc từ room_bookings.
...
BEGIN
  SELECT deposit_amount, amount_paid,
         COALESCE(vat_rate, 0), COALESCE(service_fee_rate, 0)
  ...
  -- Subtotal được chấp nhận từ client (room+services+damage+surcharges) nhưng
  -- VAT, phí dịch vụ và tổng tiền PHẢI tính lại từ rate gốc trong booking.
  v_subtotal := COALESCE(p_subtotal, 0);
  v_vat_amount := ROUND(v_subtotal * v_vat_rate / 100);
  v_service_fee_amount := ROUND(v_subtotal * v_service_fee_rate / 100);
...
REVOKE EXECUTE ON FUNCTION public.perform_checkout(uuid, uuid, numeric, numeric, numeric, numeric, text, jsonb, numeric, date) FROM anon;
